...
2. The access token is used as a *bearer* token to authorize the user to call the Express web API protected by **Azure AD**.
## Prerequisites
...
```console
    cd msal-node-api-acrs
    npm install
```

```console
    cd msal-react-spa-acrs
    npm install
```
...
1. In the **Register an application page** that appears, enter your application's registration information:
   ...
   - In the **Redirect URI (optional)** section, select **Web** in the combo-box and enter the following redirect URI: `http://localhost:5000/admin/redirect`.
...
1. Find the key `Enter_the_Application_Id_Here` and replace the existing value with the application ID (clientId) of `msal-node-api-acrs` app copied from the Azure portal.
1. Find the key `Enter_the_Tenant_Info_Here` and replace the existing value with your Azure AD tenant ID.
1. Find the key `Enter_the_Client_Secret_Here` and replace the existing value with the key you saved during the creation of `msal-node-api-acrs` copied from the Azure portal.

### Register the spa app (msal-react-spa-acrs)
...
#### Configure the spa app (msal-react-spa-acrs) to use your app registration
...
1. Find the key `Enter_the_Application_Id_Here` and replace the existing value with the application ID (clientId) of `msal-react-spa-acrs` app copied from the Azure portal.
1. Find the key `Enter_the_Tenant_Info_Here` and replace the existing value with your Azure AD tenant ID.
...
#### Configure Known Client Applications for service (msal-node-api-acrs)

For a middle-tier web API (`msal-node-api-acrs`) to be able to call a downstream web API, the middle tier app needs to be granted the required permissions as well. However, since the middle-tier cannot interact with the signed-in user, it needs to be explicitly bound to the client app in its **Azure AD** registration. This binding merges the permissions required by both the client and the middle-tier web API and presents it to the end user in a single consent dialog. The user then consent to this combined set of permissions.
...
1. In the [Azure portal](https://portal.azure.com), navigate to your `msal-node-api-acrs` app registration, and select the **Manifest** blade.
1. In the manifest editor, change the `"knownClientApplications": []` line so that the array contains the Client ID of the client application (`msal-react-spa-acrs`) as an element of the array.
